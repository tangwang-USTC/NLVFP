"""
    Calculate Rosenbluth potentials `Ĥ(𝓋̂ )` and `Ĝ(𝓋̂ )` and their first two derivatives with boundaries
  ...
  ddHvL,dHvL,HvL,ddGvL,dGvL,GvL = HGshkarofsky(ddHvL,dHvL,HvL,ddGvL,dGvL,GvL,
                               FvL,vG,nvlevel0,nc0,nck,ocp,vth,LM,uh,ns)
...
function HGshkarofsky(ddHvL::AbstractArray{T,N},dHvL::AbstractArray{T,N},
    ...
    vth::Vector{Float64},LM::Vector{Int64},uha::AbstractVector{T},ns::Int64) where{T<:Real,Tb, N}
    ...
    for isp in nsp_vec
        ...
        for L1 = 1:LM[iFv]+1
            ...
            ddHvL[:,L1,isp], dHvL[:,L1,isp], HvL[:,L1,isp], ddGvL[:,L1,isp],
                   ...
                   GvL[:,L1,isp],FLn,va,nvlevel0,nc0,nck,ocp,L1,uh)
        end
    end
...
"""
  ...
  Outputs:
            dHvL,HvL, ddGvL,dGvL,GvL = HGshkarofsky(dHvL,HvL,ddGvL,dGvL,GvL,
                                   FvL,va,nvlevel0,nc0,nck,ocp,LM,uh)
      ddHvL,dHvL,HvL, ddGvL,dGvL,GvL = HGshkarofsky(ddHvL,dHvL,HvL,ddGvL,dGvL,
                                   GvL,FvL,va,nvlevel0,nc0,nck,ocp,LM,uh)
...
function HGshkarofsky(ddHvL::AbstractArray{T,N},dHvL::AbstractArray{T,N},HvL::AbstractArray{T,N},
    ...
    for L1 in 1:LM+1
        ddHvL[:,L1], dHvL[:,L1], HvL[:,L1], ddGvL[:,L1],dGvL[:,L1],
               ...
               dGvL[:,L1],GvL[:,L1],FvL[:,L1],va,nvlevel0,nc0,nck,ocp,L1,uh)
    end
    return ddHvL,dHvL,HvL,ddGvL,dGvL,GvL
end
...
"""
  ...
  Outputs:
    dHvL,HvL,ddGvL,dGvL,GvL = HGshkarofsky(dHvL,HvL,ddGvL,dGvL,GvL,FvL,va,nvlevel0,
                                          nc0,nck,ocp,LM,uh,FvLa,vaa,nvlevel0a,ncF)
...
function HGshkarofsky(dHvL::AbstractArray{T,N},HvL::AbstractArray{T,N},
    ddGvL::AbstractArray{T,N},dGvL::AbstractArray{T,N},GvL::AbstractArray{T,N},
    FvL::AbstractArray{T,N},va::AbstractVector{Tb},nvlevel0::AbstractVector{Int64},
    nc0::Int64,nck::Int64,ocp::Int64,LM::Int64,
    uh::T,FvLa::AbstractVector,vaa::AbstractVector{T},
    ...
    for L1 in 1:LM+1
        dHLn,HLn,ddGLn,dGLn,GLn = HGshkarofsky(dHLn,HLn,ddGLn,dGLn,GLn,
                    ...
                    [nvlevel0;nvlevel0a[2:end].+(nc0-1)],nva,nvak,ocp,L1,uh)
...
"""
  ...
  Outputs:
      ddHLn,dHLn,HLn, ddGLn,dGLn,GLn = HGshkarofsky(ddHLn,dHLn,HLn,ddGLn,dGLn,GLn,
                                          FLn,va,nvlevel0,nc0,nck,ocp,L1,uh)
      dHLn,HLn, ddGLn,dGLn,GLn = HGshkarofsky(dHLn,HLn,ddGLn,dGLn,GLn,
                                          FLn,va,nvlevel0,nc0,nck,ocp,L1,uh)
...
function HGshkarofsky(dHLn::AbstractVector{T},HLn::AbstractVector{T},
    ...
    nc0::Int64,nck::Int64,ocp::Int64,L1::Int64,uh::T) where{T<:Real,Tb}

    va0 = va[nvlevel0]
    if L1 == 1
        ILn1FL0,IL1FL0,JLFL0n1,JL0FL0 = zeros(T,nc0),zeros(T,nc0),zeros(T,nc0),zeros(T,nc0)
        ...
        # @show L1, (JLFL0n1[1]/JLFL0n1bc - 1),(JL0FL0[1]/JL0FL0bc - 1), JL0FL0[1]
        if is_IJ_bc
            JLFL0n1 .+= (JLFL0n1bc - JLFL0n1[1])
            JL0FL0 .+= (JL0FL0bc - JL0FL0[1])
        end
        HLn[:] = ILn1FL0 + JLFL0n1
        dHLn[:] = - ILn1FL0 ./ va0

        GLn[:] = ((IL1FL0 + JLFL0n1) / 3 + ILn1FL0) .* va0.^2 + JL0FL0
        dGLn[:] = (ILn1FL0 + (2 * JLFL0n1 - IL1FL0) / 3.0) .* va0
        ddGLn[:] = (IL1FL0 + JLFL0n1) * (2 / 3)
        if va0[1] == 0.0
            dHLn[1] = 0.0
        end
    elseif L1 == 2
        ...
        # @show L1,(JL1n2FL0[1]/JL1n2FL0bc - 1),(JLn1FL0[1]/JLn1FL0bc - 1), JLn1FL0[1]
        if is_IJ_bc
            JL1n2FL0 .+= (JL1n2FL0bc - JL1n2FL0[1])
            JLn1FL0 .+= (JLn1FL0bc - JLn1FL0[1])
        end
    ...
    elseif L1 == 3
        ...
        # @show L1,(JLn2FL0[1]/JLn2FL0bc - 1), JLFL0bc[1]
        if is_IJ_bc
            JLn2FL0 .+= (JLn2FL0bc - JLn2FL0[1])
        end
