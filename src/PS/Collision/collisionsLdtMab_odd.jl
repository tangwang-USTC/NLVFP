...
"""
  ...
  Outputs:
    dtk = dtMcab!(Rc,edtnIKTs,err_Rc2,Mhck,nvG,ocp,vGdom,LMk,LM1,
    ...
    dtk = dtMcab2!(Rc,nvG,ocp,vGdom,LMk,LM1,naik,uaik,vthik,
...
function dtMcab!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},
    Rc2::AbstractArray{T,N},edtnIKTs2::AbstractArray{T,N2},CRDn::AbstractVector{T},
    err_Rc2::AbstractArray{T,N},DThk2::AbstractVector{T},Mhck::Vector{Matrix{T}},nMjMs::Vector{Int64},
    ...
    nak::AbstractVector{T},uak::AbstractVector{T},vathk::AbstractVector{T},ns::Int64,DThk::AbstractVector{T},
    ...
    dtk = submoment!(naik1, uaik1, vthik1, nModk1, 
            naik, uaik, vthik, nModk, Mhck, nMjMs, ns,
            Nspan_optim_nuTi,dtk;
    ...
    dtk = dtMcabn!(Rc,edtnIKTs,Rc2,edtnIKTs2,CRDn,err_Rc2,DThk2,Mc,Mc2,nMjMs,
           nvG,ocp,vGdom,LM,LM1,
           naik2,uaik2,vthik2,naik,uaik,vthik,nModk,
           CΓ,εᵣ,ma,Zq,spices,nak,uak,vathk,ns,DThk,dtk;
           is_normal=is_normal,restartfit=restartfit,maxIterTR=maxIterTR,
           autodiff=autodiff,factorMethod=factorMethod,show_trace=show_trace,
           p_tol=p_tol,f_tol=f_tol,g_tol=g_tol,n10=n10,dnvs=dnvs,
           rel_dfLM=rel_dfLM, abs_dfLM=abs_dfLM, is_LM1_full=is_LM1_full,
...
# [nMod,ns≥3], `dtk_order_Rc = :min` and `is_dtk_order_Rcaa = false`
function dtMcab!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},
    Rc2::AbstractArray{T,N},edtnIKTs2::AbstractArray{T,N2},CRDn::AbstractVector{T},
    err_Rc2::AbstractArray{T,N},DThk2::AbstractVector{T},Mhck::Vector{Matrix{T}},nMjMs::Vector{Int64},
    nvG::Vector{Int64}, ocp::Vector{Int64}, vGdom::AbstractArray{T,N2}, LM::Vector{Int64}, LM1::Int64,
    ...
    # Updating the rate of change of the kinetic moments according to parameters `nai, uai, vthi`
    dtk = dtMcabn!(Rc,edtnIKTs,Rc2,edtnIKTs2,CRDn,err_Rc2,nMjMs,DThk2,
           nvG,ocp,vGdom,LM,LM1,
           naik2,uaik2,vthik2,naik,uaik,vthik,nModk,
           CΓ,εᵣ,ma,Zq,spices,nak,uak,vathk,ns,DThk,dtk;
           is_normal=is_normal,restartfit=restartfit,maxIterTR=maxIterTR,
...
#              `dtk_order_Rc = :min` and `is_dtk_order_Rcaa = true`
function dtMcab!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},err_Rc::AbstractArray{T,N},
    Mhck::Vector{Matrix{T}},nMjMs::Vector{Int64},
    nvG::Vector{Int64}, ocp::Vector{Int64}, vGdom::AbstractArray{T,N2}, LM::Vector{Int64}, LM1::Int64,
    naik1::Vector{AbstractVector{T}},uaik1::Vector{AbstractVector{T}},vthik1::Vector{AbstractVector{T}},nModk1::Vector{Int64},
    naik::Vector{AbstractVector{T}},uaik::Vector{AbstractVector{T}},vthik::Vector{AbstractVector{T}},nModk::Vector{Int64},
    ...
    DThk::AbstractVector{T},Nspan_optim_nuTi::AbstractVector{T},dtk::T;
    ...
    # Updating the rate of change of the kinetic moments according to parameters `nai, uai, vthi`
    dtk = dtMcab2!(Rc,edtnIKTs,err_Rc,Mc,nMjMs,
...
# [nMod,ns=2], `dtk_order_Rc = :min` and `is_dtk_order_Rcaa = false`
function dtMcab!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},err_Rc::AbstractArray{T,N},
    Mhck::Vector{Matrix{T}},nMjMs::Vector{Int64},
    nvG::Vector{Int64}, ocp::Vector{Int64}, vGdom::AbstractArray{T,N2}, LM::Vector{Int64}, LM1::Int64,
    naik1::Vector{AbstractVector{T}},uaik1::Vector{AbstractVector{T}},vthik1::Vector{AbstractVector{T}},nModk1::Vector{Int64},
    ...
    # Updating the rate of change of the kinetic moments according to parameters `nai, uai, vthi`
    dtk = dtMcab2!(Rc,edtnIKTs,err_Rc,nMjMs,
           nvG,ocp,vGdom,LM,LM1,naik,uaik,vthik,nModk,
           CΓ,εᵣ,ma,Zq,spices,nak,uak,vathk,DThk,dtk;
           is_normal=is_normal,restartfit=restartfit,maxIterTR=maxIterTR,
           autodiff=autodiff,factorMethod=factorMethod,show_trace=show_trace,
...
function dtMcabn!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},
    ...
    for isp in 1:ns-1
        ...
        for iFv in isp+1:ns
            ...
            if prod(nModk[vec]) == 1
                dtk = dtMcab2!(Rc2,edtnIKTs2,err_Rc2,nMjMs[vec],
                ...
                Mc2[:,:,2] = Mc[:,:,iFv]
                dtk = dtMcab2!(Rc2,edtnIKTs2,err_Rc2,Mc2,nMjMs[vec],
                        nvG[vec],ocp[vec],vGdom[:,vec],LMk[vec],LM1,
...
function dtMcabn!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},
    Rc2::AbstractArray{T,N},edtnIKTs2::AbstractArray{T,N2},CRDn::AbstractVector{T},
    err_Rc2::AbstractArray{T,N},DThk2::AbstractVector{T},nMjMs::Vector{Int64},
    naik2::Vector{AbstractVector{T}},uaik2::Vector{AbstractVector{T}},vthik2::Vector{AbstractVector{T}},
    nvG::Vector{Int64}, ocp::Vector{Int64}, vGdom::AbstractArray{T,N2}, LMk::Vector{Int64}, LM1::Int64,
    naik::Vector{AbstractVector{T}},uaik::Vector{AbstractVector{T}},vthik::Vector{AbstractVector{T}},nModk::Vector{Int64},
    CΓ::T,εᵣ::T,ma::AbstractVector{T},Zq::AbstractVector{Int64},spices::Vector{Symbol},
...
# [nMod,ns=2], `dtk_order_Rc ∈ [:mid, :max]`
#              `dtk_order_Rc = :min` and `is_dtk_order_Rcaa = true`
function dtMcab2!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},err_Rc::AbstractArray{T,N},
    ...
    aa = Rc[1:njMs,:,:]
    if gridv_type == :uniform
        Rcsd2l!(aa,err_Rc,δtfh,vhe,nMjMs,ma.*nak,vathk,LMk,ns;is_renorm=is_renorm)
    elseif gridv_type == :chebyshev
        if is_dtf_MS_IK
            Rcsd2l!(aa,err_Rc,δtfh,vhe,nvG[1],nMjMs,ma.*nak,vathk,LMk,ns;
                      is_renorm=is_renorm,is_norm_error=is_norm_error)
        else
            Rcsd2l!(aa,err_Rc,δtfh,vhe,nvG,nMjMs,ma.*nak,vathk,LMk,ns;
                      is_renorm=is_renorm,is_norm_error=is_norm_error)
        end
    end
    Rc[1:njMs,:,:] = aa
...
#              `dtk_order_Rc = :min` and `is_dtk_order_Rcaa = false`
function dtMcab2!(Rc::AbstractArray{T,N},edtnIKTs::AbstractArray{T,N2},err_Rc::AbstractArray{T,N},nMjMs::Vector{Int64},
    nvG::Vector{Int64}, ocp::Vector{Int64}, vGdom::AbstractArray{T,N2}, LMk::Vector{Int64}, LM1::Int64,
    naik::Vector{AbstractVector{T}},uaik::Vector{AbstractVector{T}},vthik::Vector{AbstractVector{T}},nModk::Vector{Int64},
    CΓ::T,εᵣ::T,ma::AbstractVector{T},Zq::AbstractVector{Int64},spices::Vector{Symbol},
    nak::AbstractVector{T},uak::AbstractVector{T},vathk::AbstractVector{T},
