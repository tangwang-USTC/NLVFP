"""
  The `ℓᵗʰ`-order coefficients of normalized distribution function will be:
    ...
    superRBFv(ℓ,ξ) = 1 / √ξ * besseli(1/2 + ℓ, ξ) / (û * v̂ / v̂ₜₕ²)^ℓ
...
# modelDMc(μu,p,ℓ::Int) = μu^ℓ * (ℓ + 0.5)  * sqrt2pi / p^3 / sqrt2
modelDMexp(v,μu,p,ℓ::Int) = modelDMc(μu,p[1],ℓ) / (p[2] / p[1]^2).^(ℓ+0.5) *
...
modelDMexpv0(v::Vector{Float64},μu,p,ℓ) = modelDMv0c(μu,p[1],ℓ) * 2.0^ℓ * exp.(- ((v.^2 .+ p[2]^2) / p[1]^2))
modelDMexpv0(v::Float64,μu,p,ℓ) = modelDMv0c(μu,p[1],ℓ) * 2.0^ℓ * exp(- ((v^2 + p[2]^2) / p[1]^2))
