"""
  The `ℓᵗʰ`-order coefficients of normalized distribution function will be:
    ...
    superRBFv(ℓ,ξ) = 1 / √ξ * besseli(1/2 + ℓ, ξ) / (v̂ / v̂ₜₕ)^ℓ
...
modelDMexp(v,μu,p,ℓ::Int) = modelDMc(μu,p[1],ℓ) * p[1]^(ℓ+1) / (2p[2]).^0.5 *
...
modelDMexpv0(v::Vector{Float64},μu,p,ℓ::Int) = modelDMv0c(μu,p[1],ℓ) * (2p[2] / p[1])^ℓ *
                                                 exp.(- ((v.^2 .+ p[2]^2) / p[1]^2))
modelDMexpv0(v::Float64,μu,p,ℓ::Int) = modelDMv0c(μu,p[1],ℓ) * (2p[2] / p[1])^ℓ * exp(- ((v^2 + p[2]^2) / p[1]^2))
